...
- **ОБЯЗАТЕЛЬНО ПРОВЕРЯЙ БАЛАНС USDT** перед покупкой - покупай только если есть достаточно средств
- Если баланс USDT < 10 USDT → используй "pause" и объясни недостаток средств
- Если `last < low24h + 0.2%` → можно купить на 20% от доступного баланса USDT.
- Если `last > high24h - 0.2%` → максимум 5% от доступного баланса USDT.
- Если цена внутри диапазона и высокая волатильность/объём → до 10% от доступного баланса USDT.
...
БЕЗОПАСНОСТЬ:
- Не покупай, если:
  - **баланс USDT < 10 USDT** → используй "pause" с объяснением недостатка средств;
...
Обнови свой анализ и прими торговое решение на основе ДОСТУПНЫХ данных. Ответь в формате JSON."""
    ...
    def get_orders_check_prompt(self) -> str:
        ...
        """
        return """Ты — профессиональный трейдер, который проверяет активные ордера каждые 30 минут.

ТВОЯ ЗАДАЧА:
1. Анализировать активные ордера
2. Принимать решения об отмене ордеров или продаже активов
3. Обеспечивать управление рисками

ПРАВИЛА ПРОВЕРКИ ОРДЕРОВ:
- Если ордер висит более 30 минут → ОТМЕНИТЬ
- Если цена сильно изменилась от цены ордера → ОТМЕНИТЬ
- Если есть BTC на балансе и нет активных ордеров → ПРОДАТЬ ВСЕ
- Если баланс USDT слишком низкий → ПРОДАТЬ ЧАСТЬ BTC
...
- Сторона: {order.get('side', 'N/A')}
- Цена: {order.get('px', 'N/A')}
- Размер: {order.get('sz', 'N/A')}
- Состояние: {order.get('state', 'N/A')}
...
ТЕКУЩАЯ ЦЕНА: {market_data.indicators.get('current_price', 'N/A')}

Проанализируй ордера и прими решение. Ответь в формате JSON."""
